...
export const IdeaDetail: React.FC<IdeaDetailProps> = ({ idea, loading, loadingStatus, onSaveIdea, onUpdateIdea, isSaved }) => {
  ...
  const modalShareMenuRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (modalShareMenuRef.current && !modalShareMenuRef.current.contains(event.target as Node)) {
...
*Report generated by IdeaBrowser AI on ${new Date().toLocaleString()}*
  ...
  const handleShare = (platform: 'twitter' | 'linkedin' | 'email' | 'copy') => {
    const summaryText = `Check out this business idea: ${editedIdea.title}\n\n${editedIdea.description}\n\nMarket Trend: ${editedIdea.trendKeyword} (${editedIdea.trendGrowth} growth)`;
    const fullReport = generateReportContent();
    ...
    switch (platform) {
      ...
      case 'linkedin':
        ...
        break;
      case 'email':
        const emailBody = `${summaryText}\n\nLink: ${url}`;
        window.open(`mailto:?subject=${encodeURIComponent('Business Idea: ' + editedIdea.title)}&body=${encodeURIComponent(emailBody)}`, '_top');
        break;
      case 'copy':
        navigator.clipboard.writeText(fullReport);
        alert("Full market trend report copied to clipboard!");
        break;
    }
    setIsShareOpen(false);
  };
  ...
  const handleDownloadDossier = () => {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `${editedIdea.title.replace(/[^a-z0-9]/gi, '_').toLowerCase()}_comprehensive_dossier.md`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
  ...
  const handleSaveReport = () => {
      if (modalContent && idea) {
          ...
          const updatedIdea = {
              ...editedIdea,
              savedReports: [...(editedIdea.savedReports || []), newReport]
          };
          setEditedIdea(updatedIdea);
          onUpdateIdea(updatedIdea);
          alert("Report saved to this idea!");
      }
  };

  const handleModalShare = (platform: 'twitter' | 'linkedin' | 'email' | 'copy') => {
      if (modalContent) {
          const text = modalContent.content;
          const title = modalContent.title;
          const url = window.location.href;

          switch (platform) {
            case 'twitter':
              window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(title + '\n\n' + text.substring(0, 200) + '...')}`, '_blank');
              break;
            case 'linkedin':
              window.open(`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(url)}`, '_blank');
              break;
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                  ...
                  <>
                    ...
                    <div className="relative" ref={shareMenuRef}>
                        <button 
                            ...
                            className="text-slate-600 hover:text-blue-600 font-medium text-sm flex items-center gap-1 bg-white border border-slate-200 px-3 py-1.5 rounded-full shadow-sm hover:shadow-md transition-all"
                        >
                            <Share2 size={16} /> Share
                        </button>
                        {isShareOpen && (
                            <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-lg border border-slate-100 py-2 z-50">
                                <button onClick={() => handleShare('twitter')} className="w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2">
                                    <Twitter size={16} className="text-blue-400" /> Twitter
                                </button>
                                <button onClick={() => handleShare('linkedin')} className="w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2">
                                    <Linkedin size={16} className="text-blue-700" /> LinkedIn
                                </button>
                                <button onClick={() => handleShare('email')} className="w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2">
                                    <Mail size={16} className="text-slate-500" /> Email
                                </button>
                                <hr className="my-1 border-slate-100" />
                                <button onClick={() => handleShare('copy')} className="w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2">
                                    <Copy size={16} className="text-slate-500" /> Copy Text
                                </button>
                            </div>
                        )}
                    </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="space-y-8">
              ...
              {/* Saved Reports */}
              {editedIdea.savedReports && editedIdea.savedReports.length > 0 && (
                  <div className="mt-8 pt-6 border-t border-slate-100">
                      <h3 className="font-bold text-slate-900 mb-4 flex items-center gap-2 text-xl">
                          <Bookmark size={20} className="text-green-500" /> Saved Reports
                      </h3>
                      <div className="space-y-3">
                          {editedIdea.savedReports.map((report, idx) => (
                              <div key={idx} className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm flex items-center justify-between group hover:border-green-200 transition-colors">
                                  <div>
                                      <h4 className="font-bold text-slate-800 text-sm">{report.title}</h4>
                                      <div className="text-xs text-slate-400 mt-1">{report.date}</div>
                                  </div>
                                  <button 
                                      onClick={() => {
                                          setActiveModal('content');
                                          setModalContent({ title: report.title, content: report.content });
                                      }}
                        ...
                        {activeModal === 'content' && (
                            <div className="flex items-center gap-2">
                                <button 
                                    ...
                                    className="p-2 hover:bg-slate-100 rounded-full text-slate-400 hover:text-green-600 disabled:opacity-50" 
                                    title="Save Report"
                                >
                                    <Bookmark size={18} />
                                </button>
                                <div className="relative" ref={modalShareMenuRef}>
                                    <button 
                                    ...
                                    {isModalShareOpen && (
                                        <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-lg border border-slate-100 py-2 z-50">
                                            <button onClick={() => handleModalShare('twitter')} className="w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2">
                                                <Twitter size={16} className="text-blue-400" /> Twitter
                                            </button>
                                            <button onClick={() => handleModalShare('linkedin')} className="w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2">
                                                <Linkedin size={16} className="text-blue-700" /> LinkedIn
                                            </button>
                                            <button onClick={() => handleModalShare('email')} className="w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2">
                                                <Mail size={16} className="text-slate-500" /> Email
                                            </button>
                                            <hr className="my-1 border-slate-100" />
                                            <button onClick={() => handleModalShare('copy')} className="w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2">
                                                <Copy size={16} className="text-slate-500" /> Copy Text
                                            </button>
                                        </div>
                                    )}
